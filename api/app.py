...
# MongoDB 설정
mongo_url = "mongodb+srv://jtube0825:O6U6y8Jho2OZgv7C@cluster0.coc1ywm.mongodb.net/"
client = MongoClient(mongo_url)
db = client.schoolmate_db
...
def create_post(title, content):
    ...
    try:
        post = {
            ...
            "author": "익명"  # 익명 게시판이므로 모든 글은 "익명"
        }
        result = posts_collection.insert_one(post)
        return result.inserted_id
    except Exception as e:
        logging.error(f"게시글 생성 오류: {str(e)}")
        return None
...
def community():
    ...
    if post_id:
        # 특정 게시글 상세 보기
        detail_post = get_post_by_id(post_id)
        if detail_post:
...
def get_posts_api():
    ...
    try:
        ...
        logging.error(f"게시글 목록 조회 중 오류 발생: {str(e)}")
        return jsonify({
            'success': False,
...
def get_post_api(post_id):
    ...
    try:
        ...
        logging.error(f"게시글 조회 중 오류 발생: {str(e)}")
        return jsonify({
            'success': False,
...
def create_post_api():
    ...
    try:
        ...
        logging.error(f"게시글 생성 중 오류 발생: {str(e)}")
        return jsonify({
            'success': False,
